...
class GenesisTransform(object):
    ...
    def __call__(self, sample:dict) -> dict:
        ...
        # Flip
        image, target = data_augmentation(image, target, self.flip_rate)
        # Local Shuffle Pixel
        image = local_pixel_shuffling(image, prob=self.local_rate)
        # Apply non-Linear transformation with an assigned probability
        image = nonlinear_transformation(image, self.nonlinear_rate)
        # Inpainting & Outpainting
        ...
        while image.ndim < len(orig_shape):
            image = image.unsqueeze(0)

        while target.ndim < len(orig_shape):
            target = target.unsqueeze(0)

        return {'image': image, 'target': target}
# %%
""" genesis = GenesisTransform()

image = torch.rand(1, 100,100,100)
target = torch.rand(1, 100,100,100)

new = genesis({'image': image, 'target': target}) """
# %%
