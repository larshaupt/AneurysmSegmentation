...
class Predictor:
    ...
    def load_model(self, exp_name):
        ...
        try:
            ...
            self.num_classes.append(exp_config.num_classes)
    ...
    def predict(self, data, exp_name, postprocessing=False, softmax=True):
        ...
        with torch.no_grad():
            ...
            pred = model(data)
        if exp_config.num_classes > 1 and softmax:
    ...
    def predict_all(self, data):
        preds = {}
        for exp_name in self.exp_names:
            preds[exp_name] = self.predict(data, exp_name, postprocessing=False, softmax=True)
    ...
    def postprocess(self, pred, data , exp_config):
        if hasattr(exp_config, "tf_post"):
            ...
            sample = {'image': data.squeeze(0), 'pred': pred.squeeze(0)}
    ...
    def predict_all_combine(self,data, aggregation_strategy='mean', factor = 1.0, softmax=True):
        cpu_device = torch.device('cpu')
        if all(x == self.num_classes[0] for x in self.num_classes):
            num_classes = self.num_classes[0]
        else:
            num_classes = 1
        pred = torch.zeros((1,  num_classes, *data.shape[-3:]), dtype=torch.float32, device=cpu_device)
        
        for exp_name in self.exp_names:
            pred_t = self.predict(data, exp_name, postprocessing=False, softmax=softmax).to(cpu_device)
            # check if all elements in list are equal
            if num_classes==1: # only takes aneurysm class if not all predictor predict the same number of classes
                if pred_t.shape[1] == 3:
            ...
            if aggregation_strategy == 'mean':
                ...
                pred = torch.max(pred, pred_t)
            elif aggregation_strategy == 'majority':
                pred += (pred_t > 0.5).float()
        if aggregation_strategy == 'mean' or aggregation_strategy == 'majority':
            pred = pred/len(self.exp_names) * factor
